...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def union(that: TweetSet): TweetSet = this.right union (this.left union that.incl(elem))

  def descendingByRetweet: TweetList = {
    def desc(tweetSet: TweetSet, acc: TweetList): TweetList = {
      if (tweetSet.isEmpty) acc
      else desc(tweetSet.remove(tweetSet.mostRetweeted), new Cons(tweetSet.mostRetweeted, acc))
    }

    def reverseTweetList(tweetList: TweetList, acc: TweetList): TweetList = {

      if (tweetList.isEmpty) acc
      else reverseTweetList(tweetList.tail, new Cons(tweetList.head, acc))
    }

    reverseTweetList(desc(this, Nil), Nil)
  }
